...
class Pick15(Game):
    """Class that represents the Pick15 game"""
    ...
    def __init__(self, first_player: Player = None, second_player: Player = None):
        ...
        state = Pick15State(self.first_player, self.second_player)

        super().__init__(state)


class Pick15Move(Move):
    """
    Class that represents a move in the Pick15Move game

    Variables:
        number: selected number (from 1 to 9)
    """
    ...
    def __eq__(self, o: object) -> bool:
        if not isinstance(o, Pick15Move):
            return False
        return self.number == o.number


class Pick15State(State):
    ...
    def get_moves(self) -> Iterable[Pick15Move]:
        return [Pick15Move(number) for number in range(1, self.MAX_NUMBER + 1) if number not in self.selected_numbers]

    def make_move(self, move: Pick15Move) -> 'Pick15State':
    ...
    def is_finished(self) -> bool:
        return self._check_if_sums_to_15(self.current_player_numbers) or \
               self._check_if_sums_to_15(self.other_player_numbers) or \
               len(self.selected_numbers) == self.MAX_NUMBER

    def get_winner(self) -> Optional[Player]:
        if not self.is_finished():
            return None
        if self._check_if_sums_to_15(self.current_player_numbers):
            return self._current_player
        elif self._check_if_sums_to_15(self.other_player_numbers):
    ...
    def _check_if_sums_to_15(self, numbers: List[int]) -> bool:
        return self.AIM_VALUE in [sum(i) for i in itertools.combinations(numbers, 3)]
